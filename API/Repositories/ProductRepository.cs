    ...
    public class ProductRepository
    {
        ...
        public int CreateProduct(Product product, string id)
        {
            ...
            try
            {
                ...
                sql.CommandText = "INSERT INTO tbl_product (Id, Name, Price, Quantity, Address, Type, IdShop) VALUES (@id, @name, @price, @quantity, @address, @type, @idShop)";
            ...
            try
            {
                ...
                if (reader1.HasRows)
                {
                    while (reader1.Read())
                    {
                        ...
                        var address = reader1.GetString(4);
                        var type = reader1.GetString(5);
                        Product product = new Product { Id = id, Name = name, Price = price, Quantity = quantity, Address = address, Type = type };
        ...
        public int ChangeProductById(Product product)
        {
            ...
            try
            {
                ...
                var sql1 = new MySqlCommand();
                sql1.Connection = connect1;
                string queryString1 = "UPDATE tbl_product SET Name = @name, Price = @price, Quantity = @quantity, Address = @address, Type = @type WHERE Id = @id";
