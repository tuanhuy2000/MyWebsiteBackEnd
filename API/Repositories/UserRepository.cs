...
namespace API.Repositories
{
    public class UserRepository
    {
        ...
        private string EncryptPassword(string pass)
        {
            ...
            byte[] keyArray;
            byte[] toEncryptArray = UTF8Encoding.UTF8.GetBytes(pass);

            if (useHashing)
            {
                MD5CryptoServiceProvider hashmd5 = new MD5CryptoServiceProvider();
                keyArray = hashmd5.ComputeHash(UTF8Encoding.UTF8.GetBytes(_configuration["EncryptionKey"]));
            }
            else
                keyArray = UTF8Encoding.UTF8.GetBytes(_configuration["EncryptionKey"]);
